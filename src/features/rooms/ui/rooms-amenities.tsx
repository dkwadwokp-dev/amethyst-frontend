...
const RoomsAmenities = () => {
  ...
  return (
    <Section className="bg-white py-24">
      <SectionHeading subtitle="WHAT TO EXPECT" title="ROOM AMENITIES" className="mb-16" />
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
        {items.map((item, i) => (
          <div key={i} className="bg-[#F8F9FA] p-12 flex flex-col items-center justify-center text-center shadow-sm">
            <div className="text-gray-900 mb-6">{item.icon}</div>
            <div className="w-6 h-[1.5px] bg-gray-300 mb-6"></div>
            <h4 className="font-bold text-[11px] tracking-widest text-gray-900 uppercase">{item.title}</h4>
          </div>
        ))}
      </div>
    </Section>
