...
const EventForm = ({ initialData, onSubmit, title }: EventFormProps) => {
  ...
  const addTicket = () => {
    setFormData({
      ...formData,
      tickets: [...formData.tickets, { type: "", price: "" }],
    });
  };

  const removeTicket = (index: number) => {
    const newTickets = formData.tickets.filter(
      ...
      >
        ...
        <div className="space-y-2">
          ...
          <textarea
            ...
            value={formData.desc}
            onChange={handleChange}
            className="w-full border border-gray-200 p-3 text-sm focus:border-black outline-none min-h-[120px]"
            required
          ></textarea>
        </div>
        ...
        <div className="border-t border-gray-100 pt-6 space-y-4">
          ...
          {formData.tickets.map((ticket: any, index: number) => (
            <div
              key={index}
              className="grid grid-cols-[1fr_1fr_auto] gap-4 items-end bg-[#F8F9FA] p-4 border border-gray-100 relative"
            >
              <div className="space-y-2">
                <label className="text-[10px] font-bold tracking-widest text-gray-700 uppercase">
                  Ticket Type
                ...
                <input
                  ...
                  className="w-full border border-gray-200 p-3 text-sm focus:border-black outline-none bg-white"
                  required
                />
              </div>
              <div className="space-y-2">
